...
Java_com_example_tfjtest_model_NanoDet_init(JNIEnv *env, jclass clazz, jstring name, jstring path, jboolean use_gpu) { // tfj, 前两项可不管，加载模型
...
JNIEXPORT jobjectArray JNICALL
    Java_com_example_tfjtest_model_NanoDet_detect(JNIEnv *env, jclass clazz, jobject bitmap, jbyteArray image_bytes, jint width, // tfj, 前两项可不管，加载模型
    ...
    auto result = NanoDet::detector->detect(srcMatImg, (unsigned char *)imageDate, width, height, threshold, nms_threshold); // 获得结果，转换为java对应的类型，传给mainactivity使用

    srcMatImg.release();
    tempMat.release();
    env->ReleaseByteArrayElements(image_bytes, imageDate, 0);

    auto box_cls = env->FindClass("com/example/tfjtest/model/BoxInfo");
